...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wixfreto/v2-ui/master/v2-ui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/doublog/v2-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/wixfreto/blog/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/wixfreto/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
